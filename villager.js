/* villager.js - 村民生命邏輯：負責個體的遺傳、社交與生命週期運算 */
class Villager {
    constructor(cvs, gen, gender, isBaby, x, y, fName, mName, fId, mId, startAge = 0) {
        ...
        // 修正：確保 birthTime 與 age 是數字，解決 NaN 問題
        this.birthTime = totalMinutes - (Number(startAge) * CONFIG.MINS_IN_YEAR);
        this.age = Number(startAge);
        ...
        const roll = (s1, s2) => {
            ...
            let base = (s1 + s2) / 2;
            let mut = Math.floor(Math.random() * 5) - 2;
            return Math.max(3, Math.min(18, Math.floor(base + mut)));
        };
        this.str = roll(p1?.str, p2?.str); this.con = roll(p1?.con, p2?.con);
        this.siz = roll(p1?.siz, p2?.siz); this.dex = roll(p1?.dex, p2?.dex);

        this.isHero = (this.str+this.con+this.siz+this.dex >= CONFIG.HERO_TOTAL_MIN || 
                      [this.str,this.con,this.siz,this.dex].filter(s=>s>=CONFIG.HERO_PEAK_MIN).length >= CONFIG.HERO_PEAK_COUNT);
        
        this.updatePersonality(); 
        this.maxHp = Math.ceil((this.con + this.siz) / 2);
        this.hp = this.maxHp; this.hunger = 80; this.rels = {}; 
        this.mateCooldown = isBaby ? 0 : 2100; // 始祖 1 年禁慾期
    ...
    draw(ctx) {
        ...
        if(this.isHero) {
            ctx.strokeStyle = "white"; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(this.x, this.y, r + 10, 0, Math.PI * 2); ctx.stroke();
        }
        ctx.fillStyle = this.gender === "男" ? "#3498db" : "#e84393";
