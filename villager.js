...
class Villager {
    constructor(cvs, gen, gender, isBaby, x, y, fName, mName, fId, mId, startAge) {
        ...
        this.updatePersonality(); // 修正：確保性格出生即顯示
    ...
    checkHero() {
        ...
        if (total >= CONFIG.HERO_TOTAL_MIN || peaks >= CONFIG.HERO_PEAK_COUNT) {
            this.isHero = true; return true;
        }
        return false;
    }
    ...
    update() {
        ...
        if(this.age > 85 || this.hp <= 0) {
            ...
            if (this.age >= 80) updateGrace(100, `${this.name} 老死`);
            addNotice(`☠️ ${this.isElder?"長老 ":"村民 "}${this.name} 登出。`, "notice-death");
